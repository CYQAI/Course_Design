...
#define ESP8266_WIFI_INFO		"AT+CWJAP=\"HUAWEI-0404XT\",\"zz83203881\"\r\n"
...
_Bool ESP8266_SendCmd(char *cmd, char *res)
{
	...
	Usart_SendString( (char *)cmd, strlen((const char *)cmd));
...
void ESP8266_SendData(unsigned char *data, unsigned short len)
{
	...
	{
		Usart_SendString( (char*)data, len);		//发送设备连接请求数据
	}
...
void USART1_IRQHandler(void)
{
	if(RESET != usart_interrupt_flag_get(EVAL_COM1, USART_INT_FLAG_RBNE))
	{
		...
		esp8266_buf[esp8266_cnt++] = (uint8_t)usart_data_receive(EVAL_COM1);
	}
}
